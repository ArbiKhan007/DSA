...
public class Main {
   ...
   public static void printPaths(ArrayList<Edge>[]graph, int src, int dest, boolean [] vis, String psf){
      if(src == dest){
      ...
      for(Edge edge : nbrs){
         if(vis[edge.nbr] == false){
            printPaths(graph, edge.nbr, dest, vis, psf + src);
         }
      }
      vis[src] = false;
   }
   public static void printAllPath(ArrayList<Edge>[]graph , int src , int dest , int n){ 
      printPaths(graph, src, dest, new boolean[n], "");
   }
